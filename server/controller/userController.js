...
const getUserTrips = asyncHandler(async (req, res) => {
  try {
    ...
    const user = await User.findById(req?.user?._id).populate("createdTrips").populate("joinedTrips").populate("joinRequests")
    console.log(user)
    const trips = {
    ...
    logger.info('User joined trips retrieved successfully', { userId: req.user._id });
...
const getUserPublicProfile = asyncHandler(async (req, res) => {
  try {
    logger.info('Fetching public user profile', { userId: req.params.username });
    const data = await User.aggregate([
      {
        $match: {
          username: new mongoose.Types.ObjectId(req.params.username)
        }
      },
      {
        $lookup: {
